...
async function requestContent(system: string, user: string, maxTokens = 5200, temperature = 0.35): Promise<string> {
  ...
  if (!apiKey) throw new Error("请先在运行配置中填写 API Key");
...
const systemPrompt = `你是高校集体活动策划与执行 Agent。你的工作不是选择一份固定模板，而是从活动目标和现实约束出发，独立判断需要哪些执行能力，再组装成一致的方案。

输出要求：
1. 只返回一个合法 JSON 对象，不要 Markdown，不要解释；严格沿用输入草案的顶层字段结构。
2. 先建立活动画像：核心目的、正式程度、参与方式、规模、地点移动、时长密度和不确定性。不要仅凭“团建”“交流”等类型名决定流程。
3. 对交通、分组、主持、彩排、设备检查、奖品、保险、供应商、专用道具逐项做必要性判断。只有简报或现实执行条件能支持时才加入；证据不足时应省略或列为待确认，不能把大型活动配置当默认值。
4. decisions 保留 3—8 条会实质影响方案的判断结论和简短依据，例如“校内单点，不统一安排车辆”。不要输出详细思维过程。
5. 复杂度与活动相称：目标少、自由参与、规模小的活动减少岗位、会议和物料；正式呈现、多人衔接、多地点或高风险活动再增加中控、推演、安全和分组机制。
6. 先估算目标自然需要的时长。若用户给出的时段更长，不用无关环节填满；按合理时长编排，并把需要确认的差异写入 planningIssues。
7. 流程、任务、物资、预算、通知和风险必须互相一致。例如不安排统一交通，就不能出现订车任务、车辆物资和车费；不设比赛，就不能出现计分、奖品和颁奖。
8. 预算采用自下而上估算：人数 × 单价、场地/设备固定费、必要保障和少量机动。总额应在 brief.budgetMin 与 brief.budgetMax 之间，但不必花满上限；无必要的类别不要为了凑数列出。
9. 历史经验先检查迁移条件，只借用与当前目标、规模、地点和组织方式匹配的机制。reference 数组保持不变，案例中的车辆、赛制、保险、供应商等不能无条件复制。
10. 风险只保留与当前场景相关的 2—5 项，必须写可观测触发条件和具体动作；不得用泛泛的“注意安全”占位。
11. 文案自然、具体、可执行；通知与最终方案一致，可以直接发送。`;
...
export async function generateWithModel(
  ...
  const first = planSchema.parse(parseJsonObject(await requestContent(systemPrompt, firstPrompt))) as ActivityPlan;
  const finalized = finalizeModelPlan(first, seed, brief);
  if (!runtimeSettings.reviewEnabled) return finalized;
  try {
    const reviewed = planSchema.parse(parseJsonObject(await requestContent(reviewPrompt, JSON.stringify({ brief, plan: finalized }), 5000, 0.2))) as ActivityPlan;
    return finalizeModelPlan(reviewed, finalized, brief);
  } catch {
    return finalized;
  }
}
...
export async function refineWithModel(plan: ActivityPlan, instruction: string): Promise<ActivityPlan> {
  ...
  const result = planSchema.parse(parseJsonObject(await requestContent(systemPrompt, prompt))) as ActivityPlan;
...
export async function summarizeHistoryWithModel(files: Array<{ name: string; text: string }>): Promise<Omit<KnowledgeCase, "id" | "sourceFiles" | "imported">> {
  ...
  const content = await requestContent("你负责整理高校历史活动资料。只返回合法 JSON；信息不足时明确标注，不编造。", prompt, 2600, 0.2);
